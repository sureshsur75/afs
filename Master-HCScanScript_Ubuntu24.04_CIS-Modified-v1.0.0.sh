...
#Ensure crontab is restricted to authorized users (Automated)
{
 ...
 if [ "$l_cron_installed" != "yes" ]; then
  ...
  # /etc/cron.allow: must exist, mode 0640 or more restrictive, owner root, group root or crontab.
  ...
  # /etc/cron.deny: check condition if cron.allow does not exist
  ...
  elif [ ! -e /etc/cron.allow ] && [ ! -e /etc/cron.deny ]; then
...
#Ensure at is restricted to authorized users (Automated)
{
 ...
 if [ "$l_at_installed" != "yes" ]; then
  ...
  # /etc/at.deny must meet the same requirements if at.allow does not exist.
  if [ ! -e /etc/at.allow ] && [ -e /etc/at.deny ]; then
  ...
  elif [ ! -e /etc/at.allow ] && [ ! -e /etc/at.deny ]; then
