...
# Script   :   # Shell script 
# Title    :   # Security Health Check Scan Script for Ubuntu Linux-20.04 - CIS Benchmark_v3.0.0
...

{
 ...
 if [ -f "$l_file" ]; then
  ...
  if [ $(( 8#$l_perms & 0177 )) -eq 0 ] 2>/dev/null; then
...

{
 ...
 l_conf_val="$(grep -rPoi "^\h*$l_parameter_name\h*=\h*$l_expected\b" $l_searchloc 2>/dev/null | tail -1)"
...
##############################################################################################################
# 1.6.1
# Ensure /etc/motd is configured (Automated)
...
#!/usr/bin/env bash
{
 ...
 if [ -z "$l_output2" ]; then
  echo "Services / Configure systemd - timesynced" >> p1
...
#!/usr/bin/env bash
{
 ...
 if [ -z "$l_output2" ]; then
  echo "Services / Configure chrony" >> p1
...
{
 l_file="/etc/crontab"
 l_perms="$(stat -c '%a' $l_file 2>/dev/null)"
 l_uid="$(stat -c '%U' $l_file 2>/dev/null)"
 l_gid="$(stat -c '%G' $l_file 2>/dev/null)"
 if [ "$l_perms" = "600" ] && [ "$l_uid" = "root" ] && [ "$l_gid" = "root" ]; then
  echo "Services / Job Schedulers" >> p1
  ...
  echo "Services / Job Schedulers" >> p1
  echo "Ensure permissions on /etc/crontab are configured." >> p2
  echo "Permissions on /etc/crontab are NOT configured correctly. Mode: $l_perms Owner: $l_uid Group: $l_gid" >> p3
  echo "No" >> p4
  echo "2.4.1.2" >> p12
 fi
}
...
#Ensure permissions on /etc/cron.hourly are configured.
{
 ...
 if [ "$l_perms" = "700" ] && [ "$l_uid" = "root" ] && [ "$l_gid" = "root" ]; then
  ...
  echo "Services / Job Schedulers" >> p1
  echo "Ensure permissions on /etc/cron.hourly are configured." >> p2
  echo "Permissions on /etc/cron.hourly are NOT configured correctly. Mode: $l_perms Owner: $l_uid Group: $l_gid" >> p3
  echo "No" >> p4
  echo "2.4.1.3" >> p12
 fi
}
...
#Ensure permissions on /etc/cron.daily are configured.
{
 ...
 if [ "$l_perms" = "700" ] && [ "$l_uid" = "root" ] && [ "$l_gid" = "root" ]; then
  ...
  echo "Services / Job Schedulers" >> p1
  echo "Ensure permissions on /etc/cron.daily are configured." >> p2
  echo "Permissions on /etc/cron.daily are NOT configured correctly. Mode: $l_perms Owner: $l_uid Group: $l_gid" >> p3
  echo "No" >> p4
  echo "2.4.1.4" >> p12
 fi
}
...
#Ensure permissions on /etc/cron.weekly are configured.
{
 ...
 if [ "$l_perms" = "700" ] && [ "$l_uid" = "root" ] && [ "$l_gid" = "root" ]; then
  ...
  echo "Services / Job Schedulers" >> p1
  echo "Ensure permissions on /etc/cron.weekly are configured." >> p2
  echo "Permissions on /etc/cron.weekly/ are NOT configured correctly. Mode: $l_perms Owner: $l_uid Group: $l_gid" >> p3
  echo "No" >> p4
  echo "2.4.1.5" >> p12
 fi
}
...
#Ensure permissions on /etc/cron.monthly are configured.
{
 ...
 if [ "$l_perms" = "700" ] && [ "$l_uid" = "root" ] && [ "$l_gid" = "root" ]; then
  ...
  echo "Services / Job Schedulers" >> p1
  echo "Ensure permissions on /etc/cron.monthly are configured." >> p2
  echo "Permissions on /etc/cron.monthly are NOT configured correctly. Mode: $l_perms Owner: $l_uid Group: $l_gid" >> p3
  echo "No" >> p4
  echo "2.4.1.6" >> p12
 fi
}
...
#Ensure permissions on /etc/cron.d are configured.
{
 ...
 if [ "$l_perms" = "700" ] && [ "$l_uid" = "root" ] && [ "$l_gid" = "root" ]; then
  ...
  echo "Services / Job Schedulers" >> p1
  echo "Ensure permissions on /etc/cron.d are configured." >> p2
  echo "Permissions on /etc/cron.d/ are NOT configured correctly. Mode: $l_perms Owner: $l_uid Group: $l_gid" >> p3
  echo "No" >> p4
  echo "2.4.1.8" >> p12
 fi
}
...
#!/usr/bin/env bash
{
 ...
 if [ -z "$l_output2" ]; then
  echo "Network / Configure Network Services" >> p1
...
#!/usr/bin/env bash
{
 ...
 # Check net.ipv6.conf.all.forwarding = 0 (only if IPv6 is enabled)
...
#!/usr/bin/env bash
{
 ...
 # Check IPv6 parameters only if IPv6 is enabled
 if grep -Pqs '^\h*0\b' /sys/module/ipv6/parameters/disable; then
...
#!/usr/bin/env bash
{
 ...
 # Check net.ipv4.conf.all.rp_filter = 1 or 2
 l_parameter_name="net.ipv4.conf.all.rp_filter"
 l_running="$(sysctl $l_parameter_name 2>/dev/null | awk -F= '{print $2}' | tr -d ' ')"
 if [[ "$l_running" =~ ^[12]$ ]]; then
  l_output="$l_output $l_parameter_name is set to $l_running in running config."
 else
  l_output2="$l_output2 $l_parameter_name is set to $l_running in running config (expected 1 or 2)."
 fi
 l_conf_val="$(grep -rPois "^\h*$l_parameter_name\h*=\h*[12]\b" $l_searchloc 2>/dev/null | tail -1)"
 if [ -n "$l_conf_val" ]; then
  l_output="$l_output $l_parameter_name = 1 or 2 found in sysctl conf."
 else
  l_output2="$l_output2 $l_parameter_name = 1 or 2 not found in any sysctl conf file."
 fi

 # Check net.ipv4.conf.default.rp_filter = 1 or 2
...
#!/usr/bin/env bash
{
 ...
 if grep -Pqs '^\h*0\b' /sys/module/ipv6/parameters/disable; then
  ...
  l_output="$l_output IPv6 is disabled on this system, check is not applicable."
 fi
...
#!/usr/bin/env bash
{
 ...
 # Check ufw
 if command -v ufw > /dev/null 2>&1 && systemctl is-enabled --quiet ufw 2>/dev/null && systemctl is-active --quiet ufw 2>/dev/null; then
  l_fwcount=$((l_fwcount + 1))
  l_fwactive="$l_fwactive ufw"
 fi

 # Check nftables
 if command -v nft > /dev/null 2>&1 && systemctl is-enabled --quiet nftables 2>/dev/null && systemctl is-active --quiet nftables 2>/dev/null; then
  l_fwcount=$((l_fwcount + 1))
  l_fwactive="$l_fwactive nftables"
 fi

 # Check iptables
 if command -v iptables > /dev/null 2>&1 && systemctl is-enabled --quiet iptables 2>/dev/null && systemctl is-active --quiet iptables 2>/dev/null; then
  l_fwcount=$((l_fwcount + 1))
  l_fwactive="$l_fwactive iptables"
 fi

 if [ "$l_fwcount" -eq 1 ]; then
  l_output="A single firewall is in use:$l_fwactive"
 elif [ "$l_fwcount" -eq 0 ]; then
  l_output2="No firewall in use or unable to determine firewall status."
 else
  l_output2="Multiple firewalls are in use:$l_fwactive"
 fi

 if [ -z "$l_output2" ]; then
  echo "Host Based Firewall / Configure a single firewall utility" >> p1
...
pac="ufw"
if dpkg-query -W -f='${Status}' $pac 2>/dev/null | grep -q "ok installed"; then
 echo "Host Based Firewall / Configure UncomplicatedFirewall" >> p1
...
#!/usr/bin/env bash
{
 ...
 if dpkg-query -W -f='${Status}' nftables 2>/dev/null | grep -q "ok installed"; then
  l_output2="$l_output2 nftables is installed."
 else
  l_output="$l_output nftables is not installed."
 fi

 # Check if nftables.service is enabled
 if systemctl is-enabled nftables.service 2>/dev/null | grep -q "enabled"; then
  l_output2="$l_output2 nftables.service is enabled."
 else
  l_output="$l_output nftables.service is not enabled."
 fi

 # Check if nftables.service is active
 if systemctl is-active nftables.service 2>/dev/null | grep -q "^active"; then
  l_output2="$l_output2 nftables.service is active."
 else
  l_output="$l_output nftables.service is not active."
 fi

 if [ -z "$l_output2" ]; then
  echo "Host Based Firewall / Configure UncomplicatedFirewall" >> p1
  echo "Ensure nftables is not in use with ufw." >> p2
  echo "$l_output" >> p3
  echo "Yes" >> p4
  echo "4.2.2" >> p12
 else
...
#!/usr/bin/env bash
{
 if dpkg-query -W -f='${Status}' iptables-persistent 2>/dev/null | grep -q "ok installed"; then
  ...
  echo "iptables-persistent is installed." >> p3
  echo "No" >> p4
  echo "4.2.3" >> p12
 else
  echo "Host Based Firewall / Configure UncomplicatedFirewall" >> p1
...
#!/usr/bin/env bash
{
 ...
 # Check loopback interface accepts traffic
 if grep -q "allow all on lo" /etc/ufw/before.rules 2>/dev/null; then
  l_output="$l_output Loopback interface is configured to accept traffic."
 else
  l_output2="$l_output2 Loopback interface is not configured to accept traffic."
 fi

 # Check ufw-before-input chain denies traffic to loopback (127.0.0.0/8)
 if grep -q "\-A ufw-before-input -i lo -j ACCEPT" /etc/ufw/before.rules 2>/dev/null; then
  l_output="$l_output ufw-before-input allows loopback input."
 else
  l_output2="$l_output2 ufw-before-input does not allow loopback input."
 fi

 # Check deny in from 127.0.0.0/8
 if ufw status verbose 2>/dev/null | grep -q "127.0.0.0/8.*DENY IN"; then
  l_output="$l_output Traffic from 127.0.0.0/8 on non-loopback is denied."
 else
  l_output2="$l_output2 Traffic from 127.0.0.0/8 on non-loopback is not denied."
 fi

 # Check deny in from ::1 (IPv6)
 if ufw status verbose 2>/dev/null | grep -q "::1.*DENY IN"; then
  l_output="$l_output Traffic from ::1 on non-loopback is denied."
 else
  l_output2="$l_output2 Traffic from ::1 on non-loopback is not denied."
 fi
...
#!/usr/bin/env bash
{
 ...
 done < <(ufw show raw 2>&1 | awk '/^#/{n=1} n' | sort -u)
 ...
 while read -r l_openport; do
  a_openports+=("$l_openport")
 done < <(ss -tulnH | awk '{print $5}' | awk -F: '{print $NF}' | grep -E '^[0-9]+$' | sort -u)
 ...
 if [ -n "${a_diff[*]}" ]; then
  l_output2="The following port(s) don't have a rule in UFW: ${a_diff[*]}"   # FIX 1: was printf '%s\n' "${a_diff[@]}"
 else
 ...
 if [ -z "$l_output2" ]; then
  ...
  echo "Host Based Firewall / Configure UncomplicatedFirewall" >> p1
  echo "Ensure ufw firewall rules exist for all open ports." >> p2
  echo "$(echo "$l_output2" | tr '\n' ' ' | tr -s ' ')" >> p3   # FIX 2: was echo "$l_output2"
  echo "No" >> p4
  echo "4.2.7" >> p12
 fi
}
...
#!/usr/bin/env bash
{
 ...
 # Check default policy for incoming, outgoing and routed is deny/reject/disabled
 l_policy="$(ufw status verbose 2>/dev/null | grep -i "^Default:")"

 if echo "$l_policy" | grep -qi "incoming.*deny\|incoming.*reject\|incoming.*disabled"; then
  l_output="$l_output Default incoming policy is deny/reject/disabled."
 else
  l_output2="$l_output2 Default incoming policy is not deny/reject/disabled."
 fi

 if echo "$l_policy" | grep -qi "outgoing.*deny\|outgoing.*reject\|outgoing.*disabled"; then
  l_output="$l_output Default outgoing policy is deny/reject/disabled."
 else
  l_output2="$l_output2 Default outgoing policy is not deny/reject/disabled."
 fi

 if echo "$l_policy" | grep -qi "routed.*deny\|routed.*reject\|routed.*disabled"; then
  l_output="$l_output Default routed policy is deny/reject/disabled."
 else
  l_output2="$l_output2 Default routed policy is not deny/reject/disabled."
 fi
...
#!/usr/bin/env bash
{
 ...
 if ! dpkg-query -W -f='${Status}' ufw 2>/dev/null | grep -q "ok installed"; then
  l_output="$l_output ufw is not installed."
 else
  # If ufw is installed, check if it is inactive and ufw.service is not enabled
  if ufw status 2>/dev/null | grep -q "Status: inactive"; then
   l_output="$l_output ufw status is inactive."
  else
   l_output2="$l_output2 ufw is active."
  fi
  if systemctl is-enabled ufw.service 2>/dev/null | grep -q "masked\|disabled"; then
   l_output="$l_output ufw.service is disabled or masked."
  else
   l_output2="$l_output2 ufw.service is enabled."
  fi
 fi
...
#!/usr/bin/env bash
{
 if nft list tables 2>/dev/null | grep -q "^table"; then
  ...
  echo "nftables table exists: $(nft list tables 2>/dev/null)" >> p3
  echo "Yes" >> p4
  echo "4.3.4" >> p12
 else
  echo "Host Based Firewall / Configure nftables" >> p1
  echo "Ensure a nftables table exists." >> p2
  echo "No nftables table exists." >> p3
  echo "No" >> p4
  echo "4.3.4" >> p12
 fi
}
...
#!/usr/bin/env bash
{
 ...
 # Check IPv4 loopback traffic from 127.0.0.0/8 is configured to drop
 if nft list ruleset 2>/dev/null | awk '/hook input/,/}/' | grep -q 'ip saddr'; then
  l_output="$l_output IPv4 loopback traffic from 127.0.0.0/8 is configured to drop."
 else
  l_output2="$l_output2 IPv4 loopback traffic from 127.0.0.0/8 is not configured to drop."
 fi

 # Check IPv6 loopback traffic from ::1 is configured to drop (only if IPv6 enabled)
 if grep -Pqs '^\h*0\b' /sys/module/ipv6/parameters/disable; then
  if nft list ruleset 2>/dev/null | awk '/hook input/,/}/' | grep -q 'ip6 saddr'; then
   l_output="$l_output IPv6 loopback traffic from ::1 is configured to drop."
  else
   l_output2="$l_output2 IPv6 loopback traffic from ::1 is not configured to drop."
  fi
 fi
...
#!/usr/bin/env bash
{
 ...
 if nft list ruleset 2>/dev/null | grep -q "hook input" && nft list ruleset 2>/dev/null | awk '/hook input/,/}/' | grep -q "policy drop"; then
  l_output="$l_output INPUT chain has default policy drop."
 else
  l_output2="$l_output2 INPUT chain does not have default policy drop."
 fi

 # Check FORWARD chain has policy drop
 if nft list ruleset 2>/dev/null | grep -q "hook forward" && nft list ruleset 2>/dev/null | awk '/hook forward/,/}/' | grep -q "policy drop"; then
  l_output="$l_output FORWARD chain has default policy drop."
 else
  l_output2="$l_output2 FORWARD chain does not have default policy drop."
 fi

 # Check OUTPUT chain has policy drop
 if nft list ruleset 2>/dev/null | grep -q "hook output" && nft list ruleset 2>/dev/null | awk '/hook output/,/}/' | grep -q "policy drop"; then
  l_output="$l_output OUTPUT chain has default policy drop."
 else
  l_output2="$l_output2 OUTPUT chain does not have default policy drop."
 fi
...
#!/usr/bin/env bash
{
 ...
 # Check input base chain is configured in /etc/nftables.conf
 if [ -f /etc/nftables.conf ]; then
  if grep -qE "^\s*include" /etc/nftables.conf 2>/dev/null; then
   l_nft_conf="$(awk '/\$Item/ {print $2}' /etc/nftables.conf | sed 's/"//g')"
  fi

  # Check input base chain exists in permanent config
  if grep -rq "hook input" /etc/nftables.conf /etc/nftables.d/ 2>/dev/null; then
   l_output="$l_output Input base chain exists in permanent nftables config."
  else
   l_output2="$l_output2 Input base chain does not exist in permanent nftables config."
  fi

  # Check forward base chain exists in permanent config
  if grep -rq "hook forward" /etc/nftables.conf /etc/nftables.d/ 2>/dev/null; then
   l_output="$l_output Forward base chain exists in permanent nftables config."
  else
   l_output2="$l_output2 Forward base chain does not exist in permanent nftables config."
  fi

  # Check output base chain exists in permanent config
  if grep -rq "hook output" /etc/nftables.conf /etc/nftables.d/ 2>/dev/null; then
   l_output="$l_output Output base chain exists in permanent nftables config."
  else
   l_output2="$l_output2 Output base chain does not exist in permanent nftables config."
  fi
 else
  l_output2="$l_output2 /etc/nftables.conf does not exist."
 fi
...
#!/usr/bin/env bash
{
 ...
 if dpkg-query -W -f='${Status}' iptables 2>/dev/null | grep -q "ok installed"; then
...
#!/usr/bin/env bash
{
 ...
 if ! dpkg-query -W -f='${Status}' nftables 2>/dev/null | grep -q "ok installed"; then
  ...
  # If installed, check nftables.service is not enabled
  if systemctl is-enabled nftables.service 2>/dev/null | grep -q "masked\|disabled"; then
   l_output="$l_output nftables.service is disabled or masked."
  else
   l_output2="$l_output2 nftables.service is enabled."
  fi
  # Check nftables.service is not active
  if ! systemctl is-active nftables.service 2>/dev/null | grep -q "^active"; then
   l_output="$l_output nftables.service is not active."
  else
   l_output2="$l_output2 nftables.service is active."
...
#!/usr/bin/env bash
{
 ...
 if ! dpkg-query -W -f='${Status}' ufw 2>/dev/null | grep -q "ok installed"; then
  ...
  # If installed, check ufw status is inactive
  if ufw status 2>/dev/null | grep -q "Status: inactive"; then
   l_output="$l_output ufw status is inactive."
  else
   l_output2="$l_output2 ufw is active."
  fi
  # Check ufw.service is not enabled
  if ! systemctl is-enabled ufw.service 2>/dev/null | grep -q "^enabled"; then
   l_output="$l_output ufw.service is not enabled."
  else
   l_output2="$l_output2 ufw.service is enabled."
  fi
  # Check ufw.service is not active
  if ! systemctl is-active ufw.service 2>/dev/null | grep -q "^active"; then
   l_output="$l_output ufw.service is not active."
  else
   l_output2="$l_output2 ufw.service is active."
  fi
 fi
...
#Ensure iptables default deny firewall policy.
{
 ...
 for l_chain in INPUT OUTPUT FORWARD; do
  l_policy="$(iptables -L "$l_chain" 2>/dev/null | awk 'NR==1 {print $4}' | tr -d '()')"
  if [ "$l_policy" = "DROP" ] || [ "$l_policy" = "REJECT" ]; then
   l_output="$l_output Chain $l_chain policy is $l_policy (compliant)."
  else
   l_output2="$l_output2 Chain $l_chain policy is ${l_policy:-not set} (required: DROP or REJECT)."
  fi
 done
 if [ -z "$l_output2" ]; then
  echo "Host Based Firewall / Configure iptables" >> p1
...
#Ensure iptables loopback traffic is configured.
{
 ...
 # Check INPUT ACCEPT on lo
 if iptables -L INPUT -v -n 2>/dev/null | awk 'NR>2' | grep -q "ACCEPT.*lo.*0\.0\.0\.0"; then
  l_output="$l_output INPUT accepts traffic on loopback (lo)."
 else
  l_output2="$l_output2 INPUT rule to ACCEPT loopback (lo) traffic is missing."
 fi

 # Check INPUT DROP from 127.0.0.0/8 on non-lo
 if iptables -L INPUT -v -n 2>/dev/null | awk 'NR>2' | grep -q "DROP.*127\.0\.0\.0"; then
  l_output="$l_output INPUT drops traffic from 127.0.0.0/8 on non-lo."
 else
  l_output2="$l_output2 INPUT rule to DROP 127.0.0.0/8 on non-lo is missing."
 fi

 # Check OUTPUT ACCEPT on lo
 if iptables -L OUTPUT -v -n 2>/dev/null | awk 'NR>2' | grep -q "ACCEPT.*lo.*0\.0\.0\.0"; then
  l_output="$l_output OUTPUT accepts traffic on loopback (lo)."
 else
  l_output2="$l_output2 OUTPUT rule to ACCEPT loopback (lo) traffic is missing."
 fi

 if [ -z "$l_output2" ]; then
  echo "Host Based Firewall / Configure iptables" >> p1
  echo "Ensure iptables loopback traffic is configured." >> p2
  echo "$(echo "$l_output" | tr '\n' ' ' | tr -s ' ')" >> p3
  echo "Yes" >> p4
  echo "4.4.2.2" >> p12
 else
  echo "Host Based Firewall / Configure iptables" >> p1
...
#Ensure iptables firewall rules exist for all open ports.
{
 ...
 l_ports="$(ss -4tuln 2>/dev/null | awk '
  NR>1 {
   local_addr=$5
   if (local_addr !~ /127\.0\.0\.1:/) {
    ...
    port=a[n]
    proto=tolower($1)
    if (port ~ /^[0-9]+$/ && (proto=="tcp" || proto=="udp")) {
     print proto ":" port
    }
   }
  }' | sort -u)"

 l_rules="$(iptables -S INPUT 2>/dev/null)"

 if [ -z "$l_ports" ]; then
  l_output="No non-loopback IPv4 open ports found."
  l_status="Yes"
 elif [ -z "$l_rules" ]; then
  l_output2="iptables INPUT rules could not be read."
  l_status="No"
 else
  for entry in $l_ports; do
   l_proto="${entry%%:*}"
   l_port="${entry##*:}"

   if ! printf '%s\n' "$l_rules" | grep -Pq -- "^-A INPUT\b.*\b-p[[:space:]]+$l_proto\b.*\b--dport[[:space:]]+$l_port\b.*\b-j[[:space:]]+ACCEPT\b"; then
    if [ -z "$l_missing" ]; then
     l_missing="${l_proto}/${l_port}"
    else
     l_missing="${l_missing} ${l_proto}/${l_port}"
    fi
   fi
  done

  if [ -z "$l_missing" ]; then
   l_output="Firewall rules exist for all open IPv4 ports."
   l_status="Yes"
  else
   l_output2="Firewall rules missing for IPv4 ports: $l_missing"
   l_status="No"
  fi
 fi
 ...
 if [ "$l_status" = "Yes" ]; then
  ...
  echo "$(printf '%s' "$l_output" | tr '\n' ' ' | tr -s ' ')" >> p3
  echo "Yes" >> p4
  echo "4.4.2.4" >> p12
 else
  echo "Host Based Firewall / Configure iptables" >> p1
  echo "Ensure iptables firewall rules exist for all open ports." >> p2
  echo "$(printf '%s' "$l_output2" | tr '\n' ' ' | tr -s ' ')" >> p3
  echo "No" >> p4
  echo "4.4.2.4" >> p12
 fi
}
...
if [ -f /sys/module/ipv6/parameters/disable ] && grep -Pqs '^\h*0\h*$' /sys/module/ipv6/parameters/disable; then
 l_ipv6_enabled="yes"
elif sysctl -a 2>/dev/null | grep -Pqs '^\h*net\.ipv6\.conf\.all\.disable_ipv6\h*=\h*0\h*$' && \
     sysctl -a 2>/dev/null | grep -Pqs '^\h*net\.ipv6\.conf\.default\.disable_ipv6\h*=\h*0\h*$'; then
 l_ipv6_enabled="yes"
fi

if [ "$l_ipv6_enabled" = "yes" ]; then
 l_input_policy="$(ip6tables -S INPUT 2>/dev/null | head -n 1)"
 l_output_policy="$(ip6tables -S OUTPUT 2>/dev/null | head -n 1)"
 l_forward_policy="$(ip6tables -S FORWARD 2>/dev/null | head -n 1)"

 if echo "$l_input_policy" | grep -Pq '^-P INPUT (DROP|REJECT)$' && \
    echo "$l_output_policy" | grep -Pq '^-P OUTPUT (DROP|REJECT)$' && \
    echo "$l_forward_policy" | grep -Pq '^-P FORWARD (DROP|REJECT)$'; then
  l_output="ip6tables default deny policy is configured for INPUT/OUTPUT/FORWARD."
 else
  l_output2="ip6tables default deny policy is not configured for INPUT/OUTPUT/FORWARD."
 fi
else
...
#Ensure ip6tables loopback traffic is configured.
{
 ...
 if [ "$l_ipv6_enabled" = "yes" ]; then
  l_rules="$(ip6tables -S 2>/dev/null)"
  if [ -z "$l_rules" ]; then
   l_output2="ip6tables rules could not be read."
  else
   if echo "$l_rules" | grep -Pq '^-A INPUT -i lo -j ACCEPT$'; then
    l_output="$l_output IPv6 INPUT loopback accept rule configured."
   else
    l_output2="$l_output2 IPv6 INPUT loopback accept rule missing."
   fi
   if echo "$l_rules" | grep -Pq '^-A OUTPUT -o lo -j ACCEPT$'; then
    l_output="$l_output IPv6 OUTPUT loopback accept rule configured."
   else
    l_output2="$l_output2 IPv6 OUTPUT loopback accept rule missing."
   fi
   if echo "$l_rules" | grep -Pq '^-A INPUT -s ::1/128( .*)? -j DROP$|^-A INPUT( .*)? -s ::1/128( .*)? -j DROP$'; then
    l_output="$l_output IPv6 ::1 drop rule configured."
   else
    l_output2="$l_output2 IPv6 ::1 drop rule missing."
   fi
  fi
 else
  l_output="IPv6 is not enabled on the system."
...
#Ensure ip6tables firewall rules exist for all open ports.
{
 ...
 if [ "$l_ipv6_enabled" = "yes" ]; then
  l_open_ports="$(ss -6tuln 2>/dev/null | awk 'NR>1 && $5 !~ /%lo/ && $5 !~ /\[::1\]/ {split($1,a," "); proto=a[1]; local=$5; sub(/^.*:/,"",local); if (local ~ /^[0-9]+$/) print proto ":" local}' | sort -u)"
  if [ -z "$l_open_ports" ]; then
   l_output="No non-loopback IPv6 open ports found."
  else
   l_rules="$(ip6tables -S INPUT 2>/dev/null)"
   if [ -z "$l_rules" ]; then
    l_output2="ip6tables INPUT rules could not be read."
   else
    while IFS= read -r l_port; do
     [ -z "$l_port" ] && continue
     l_proto="${l_port%%:*}"
     l_num="${l_port##*:}"
     if ! echo "$l_rules" | grep -Pq -- "^-A INPUT .* -p $l_proto .* --dport $l_num .* -j ACCEPT\b|^-A INPUT .* --dport $l_num .* -p $l_proto .* -j ACCEPT\b"; then
      l_missing_ports="$l_missing_ports $l_proto/$l_num"
     fi
    done <<< "$l_open_ports"
    if [ -z "$l_missing_ports" ]; then
     l_output="All non-loopback IPv6 open ports have ip6tables INPUT ACCEPT rules."
    else
     l_output2="Missing ip6tables INPUT ACCEPT rules for:$(echo "$l_missing_ports" | tr '\n' ' ' | tr -s ' ')."
    fi
   fi
  fi
 else
...

{
    ...
    f_sshd_files_chk() {
        ...
        if (( (8#$l_mode & l_perm_mask) > 0 )); then
            l_output2="$l_output2 File: \"$l_file\" mode \"$l_mode\" (required: $l_maxperm or more restrictive)."
        fi

        # Check owner
        if [[ "$l_user" != "root" ]]; then
            l_output2="$l_output2 File: \"$l_file\" owned by \"$l_user\" (required: root)."
        fi
        ...
        # If compliant
        if [[ -z "$l_output2" ]]; then
            l_output="$l_output File: \"$l_file\": Correct mode ($l_mode), owner ($l_user), group ($l_group)."
        fi
    }
    ...
    # Check any configuration files under sshd_config.d
    ...
    if [[ -z "$l_output2" ]]; then
        ...
        echo "Access to /etc/ssh/sshd_config is configured correctly." >> p3
        echo "Yes" >> p4
        echo "5.1.1" >> p12
    else
        echo "Access Control / Configure SSH Server" >> p1
        echo "Ensure access to /etc/ssh/sshd_config is configured." >> p2
        echo "Access to /etc/ssh/sshd_config is NOT configured correctly." >> p3
        echo "No" >> p4
        echo "5.1.1" >> p12
    fi
}
...
{
 l_output="" l_output2=""
 l_weak_ciphers="3des-cbc|aes128-cbc|aes192-cbc|aes256-cbc"
 ...
 if [ -z "$l_current_ciphers" ]; then
  ...
  if [ -n "$l_found_weak" ]; then
   ...
   l_output="$l_output No weak ciphers found."
  fi
 fi
 if [ -z "$l_output2" ]; then
  echo "Access Control / Configure SSH Server" >> p1
...
#Ensure sshd KexAlgorithms is configured.
{
 ...
 if [ -z "$l_current_kex" ]; then
  ...
  if [ -n "$l_found_weak" ]; then
   ...
   l_output="$l_output No weak KexAlgorithms found."
  fi
 fi
 if [ -z "$l_output2" ]; then
  echo "Access Control / Configure SSH Server" >> p1
...
#!/usr/bin/env bash
{
 ...
 if dpkg-query -W -f='${Status}' aide 2>/dev/null | grep -q "ok installed"; then
  l_output="$l_output aide is installed."
 else
  l_output2="$l_output2 aide is not installed."
 fi

 # Check aide-common is installed
 if dpkg-query -W -f='${Status}' aide-common 2>/dev/null | grep -q "ok installed"; then
  l_output="$l_output aide-common is installed."
 else
  l_output2="$l_output2 aide-common is not installed."
 fi
...
#!/usr/bin/env bash
{
 ...
 # Check dailyaidecheck.timer is enabled and dailyaidecheck.service is enabled/static
 l_timer_status="$(systemctl list-unit-files | awk '/dailyaidecheck\.timer|dailyaidecheck\.service/{print $1" "$2}')"

 if echo "$l_timer_status" | grep -q "dailyaidecheck.service" && \
    echo "$l_timer_status" | grep -qE "dailyaidecheck.service.*(static|enabled)"; then
  l_output="$l_output dailyaidecheck.service is static or enabled."
 else
  l_output2="$l_output2 dailyaidecheck.service is not enabled or static."
 fi

 if echo "$l_timer_status" | grep -qE "dailyaidecheck.timer.*enabled"; then
  l_output="$l_output dailyaidecheck.timer is enabled."
 else
  l_output2="$l_output2 dailyaidecheck.timer is not enabled."
 fi

 # Check dailyaidecheck.timer is active
 if systemctl is-active dailyaidecheck.timer 2>/dev/null | grep -q "^active"; then
  l_output="$l_output dailyaidecheck.timer is active."
 else
  l_output2="$l_output2 dailyaidecheck.timer is not active."
 fi
...
#!/usr/bin/env bash
{
 ...
 # Check AIDE is configured to use cryptographic mechanisms for audit tools
 if grep -Ps -- '(\\/sbin\\/(auditctl|auditd|ausearch|aureport|autrace|augenrules))\\b' \
  /etc/aide/aide.conf /etc/aide/aide.conf.d/*.conf 2>/dev/null | \
  grep -q "p+i+n+u+g+s+b+acl+xattrs+sha512"; then
  l_output="$l_output AIDE is configured to use cryptographic mechanisms to protect audit tools."
 else
  l_output2="$l_output2 AIDE is not configured to use cryptographic mechanisms to protect audit tools."
 fi
...
#!/usr/bin/env bash
{
 ...
 # Check systemd-journald.service is enabled (should be static)
 l_enabled="$(systemctl is-enabled systemd-journald.service 2>/dev/null)"
 if echo "$l_enabled" | grep -qE "^static|^enabled"; then
  l_output="$l_output systemd-journald.service is $l_enabled."
 else
  l_output2="$l_output2 systemd-journald.service is not static or enabled (status: $l_enabled)."
 fi

 # Check systemd-journald.service is active
 ...
 if [ -z "$l_output2" ]; then
  echo "Logging and Auditing / Configure System Logging" >> p1
...
#!/usr/bin/env bash
{
 ...
 if [ -z "$l_output2" ]; then
  echo "Logging and Auditing / Configure rsyslog" >> p1
...
#!/usr/bin/env bash
{
 if dpkg-query -W -f='${Status}' rsyslog 2>/dev/null | grep -q "ok installed"; then
  ...
  echo "rsyslog is installed." >> p3
  echo "Yes" >> p4
  echo "6.2.3.2" >> p12
 else
  echo "Logging and Auditing / Configure rsyslog" >> p1
  echo "Ensure rsyslog is installed." >> p2
  echo "rsyslog is not installed." >> p3
  echo "No" >> p4
  echo "6.2.3.2" >> p12
 fi
}
...
#!/usr/bin/env bash
{
 ...
 if [ -n "$l_fcm" ]; then
  ...
  if [ -n "$l_fcm_any" ]; then
   ...
   l_output2="\$FileCreateMode is not configured in rsyslog config. Required: 0640 or more restrictive."
  fi
 fi
...
#!/usr/bin/env bash
{
 ...
 # Check advanced format - module and input directives
 l_adv1="$(grep -P '^\h*module\(load="imtcp"\)' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null | grep -v '^\s*#')"
 l_adv2="$(grep -P '^\h*input\(type="imtcp"\s+port=' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null | grep -v '^\s*#')"

 # Check obsolete legacy format
 l_leg1="$(grep -P '^\h*\$ModLoad\h+imtcp\b' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null | grep -v '^\s*#')"
 l_leg2="$(grep -P '^\h*\$InputTCPServerRun\b' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null | grep -v '^\s*#')"

 if [ -z "$l_adv1" ] && [ -z "$l_adv2" ] && [ -z "$l_leg1" ] && [ -z "$l_leg2" ]; then
  l_output="rsyslog is not configured to receive logs from a remote client."
 else
  l_output2="rsyslog is configured to receive logs from a remote client: ${l_adv1} ${l_adv2} ${l_leg1} ${l_leg2}"
 fi
...
#!/usr/bin/env bash
{
 ...
 if [ -z "$l_output2" ]; then
  echo "Logging and Auditing / Configure Logfiles" >> p1
...
#Ensure access to /etc/passwd is configured.
{
 ...
 if [ -z "$l_output2" ]; then
  echo "System Maintenance / Configure System file and directory access" >> p1
...
if [ -f "$filepw" ]; then
	...
	echo "Local User and Group Settings" >> p1
	echo "Ensure accounts in /etc/passwd use shadowed passwords." >>p2
...
#Ensure local interactive user home directories are configured.
{
 ...
 done < <(awk -v pat="$l_valid_shells" -F: '$7 ~ pat { print $1" "$6 }' /etc/passwd)
